...
// SHIPMENTS BANK DATABASE CONFIG
$dsn = 'mysql:host = localhost';
$username = 'root';
$password = '';

$host = 'localhost' ;// i added this later on the training time of electronics nikaten  after i observed the 
$dbname = 'shipmintsbank';// above 1st statement , i see that it has one less variable,


// IRESLHA.COM DATABASE CONFIG
$idsn = 'mysql:host = localhost';
$iusername = 'root';
$ipassword = '';

$ihost = 'localhost' ;// i added this later on the training time of electronics nikaten  after i observed the
$idbname = 'irselha';// above 1st statement , i see that it has one less variable,


// but before this the application was working how , by chossing the database later(which you have to do manually in each SQL statement, eg.--> INSERT INTO shipmintsbank.users (username, password, email, join_date) ->(in the signup page.)
...
//FOR IRSELHA.COM
$idb = new PDO("mysql:host=$ihost;dbname=irselha",$iusername,$ipassword, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8")); 	// now we dont want to show our errors to the public (if 													 any error  ocuurs in the connection to the database),so 													for that we use the try-catch block and we use the 															PDO-exception for handling that exception
