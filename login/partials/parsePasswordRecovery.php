...
if (isset($_POST['recovery_btn'], $_POST['token'])) {
	...
	if ($_POST['token']) {
		...
		if ( empty($form_errors) ){
			...
			$email = $_POST['email'];
			try{
				$sqlQuery = "SELECT * FROM shipmintsbank.users WHERE email=:email";
				...
				if ( $row = $statement->fetch() ) {  #  ie, if  data is fetched   ie. user exists and we have to send him the reset link to his website
					...
					$mail_body = '<html>
									<body style="background-color:#CCCCCC; color:#000; font-family: Arial, Helvetica, sans-serif; line-height:1.8em;">
										...
										<p><a href="http://localhost/projects/SHIPMENTSBANK/login/forgot_password.php?id='.$encode_id.'"> Reset Password</a></p>
										<p><strong>&copy;2019 Shipments Bank Solution</strong></p>
									</body>
								</html>';
