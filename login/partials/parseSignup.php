			...
			if ($arrayReturned['status'] == false ) {//ie no duplicasy for username found in the database,
				...
				if ($arrayReturned['status'] == false ) {//ie no duplicasy for email found in the database
					try{
						...
						if($statement->rowcount()==1){ # ie if one row is changed then ...
							$user_id = $db->lastInsertId();
							$encode_id = base64_encode("encodeUserid{$user_id}");
                            /*$headers = 'From: Shipments Bank kldrapqwb8dz@n3plcpnl0096.prod.ams3.secureserver.net' . "\r\n";
                            $headers .= 'Reply-To: kldrapqwb8dz@n3plcpnl0096.prod.ams3.secureserver.net'. "\r\n";
							...
							$mail_body = '<html>
											<body style="background-color:#CCCCCC; color:#000; font-family: Arial, Helvetica, sans-serif;
												...
												<p><a href="https://shipmintsbank.irselha.com/login/activate.php?id='.$encode_id.'&item_name='.$item_name.'"> Confirm Email</a></p>
												<p><strong>&copy;2019 Shipments Bank System </strong></p>
											</body>
									</html>';
                            /*$mail_body = 'User Authentication System
                            Thank you for registering, please click on the link below to
							confirm your email address
							https://shipmintsbank.irselha.com/login/activate.php?id='.$encode_id.'&item_name='.$item_name.'
							2019 Shipments Bank System
                            ';*/
...
//							if ($mail->Send()) {
							...
							if (mail($email, ' SHIBMENTS BANK SYSTEM', $mail_body, $headers)) {
								# popupMessage($title, $text, $type, $page)
								$result = popupMessage("Hey {$username}!!",'Hurray, registration successfull. Please check your email for conformation link!','success','login.php');
							}else{
...
//        $statement->execute(array(':activated' => "1", ':id' => $id));
        ...
        if ($statement->execute(array(':activated' => "1", ':id' => $id))) {
            ...
            //$user_id = "35";
            //$user_id = $db->lastInsertId();
            /*$sqlSubscriptionInsert = "INSERT INTO shipmintsbank.user_subscriptions (user_id, item_name)
										VALUES  (:user_id, :item_name)";*/
            $sqlSubscriptionInsert = "INSERT INTO shipmintsbank.user_subscriptions (user_id, item_name) 
										SELECT * FROM (SELECT :user_id, :item_name) AS tmp
										WHERE NOT EXISTS (
                                        SELECT user_id FROM shipmintsbank.user_subscriptions WHERE user_id = :user_id
                                        ) LIMIT 1
										";
            $subsStatement = $db->prepare($sqlSubscriptionInsert);
            $subsStatement->execute(array(':user_id'=> $user_id, ':item_name'=>$item_name));
			...
			$result = "<div class=\"container\"  style=\"padding-top:25%\"><h2>Email Confirmed: You have 10 days trial </h2>
